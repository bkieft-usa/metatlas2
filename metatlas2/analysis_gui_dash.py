...
def build_dash_app(
    ...
    def _get_ms2_scans(inchi_key, adduct, rt_min=None, rt_max=None):
        ...
        ms2_sub = ms2_df[(ms2_df["inchi_key"] == inchi_key) & (ms2_df["adduct"] == adduct)]
    ...
    def _flush_to_db(state):
        ...
        dbi.write_gui_updates_to_db(project_path, row["curation_uid"], updates)
    ...
    def save_and_exit(n_clicks, state):
        ...
        try:
            _flush_to_db(state)
            logger.info("save_and_exit: flush succeeded for compound %s", state.get("compound_idx"))
            msg = "Saved. Shutting down…"
        except Exception as exc:
            logger.exception("save_and_exit: _flush_to_db failed: %s", exc)
            msg = f"Save failed: {type(exc).__name__}: {exc}. Shutting down anyway…"

        def _shutdown():
            time.sleep(0.8)
            os._exit(0)

        threading.Timer(0.8, _shutdown).start()
        return msg, True
